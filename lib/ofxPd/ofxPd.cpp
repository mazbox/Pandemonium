...
ofxPd::ofxPd() {
	...
	sprintf(strin, "/tmp/%x.dylib", (int)this);
	string uniquePath = strin;
	printf("Instance %x opening dylib %s\n", this, strin);
	ofxCopyFile("/Users/marek/Desktop/libpdlib.dylib", uniquePath);
	pdHandle = dlopen(uniquePath.c_str(), RTLD_LAZY|RTLD_FIRST);
	...
	*/
	...
	if(!libpd_alreadyInited()) {
		libpd_init();
		libpd_printhook = (t_libpd_printhook) ofxPd_printhook;
		printf("Initializing pd\n");
	} else {
		printf("Pd already inited\n");
	}
	//init();
